...
### 2. 참여율(Engagement Rate) 0.0% 표시 이슈 (2026-01-13)

- **문제 현상**: 트렌딩 페이지에서 모든 영상의 참여율이 `0.0%`로 고정되어 표시됨.
- **원인 분석**: 
  - DB에는 참여율이 소수점 비율(예: `0.0116`)로 저장되어 있었음.
  - 프론트엔드에서는 이를 백분율로 간주하고 `toFixed(1)` 처리를 하여 `0.0%`로 표시됨.
- **해결 방법**:
  - **코드 수정**: `videos.py` 가공 로직에서 참여율 계산 시 `* 100`을 적용하여 백분율로 저장하도록 수정.
  - **데이터 보정**: SQL을 통해 기존 DB 레코드의 `engagement_rate`에 100을 곱하는 업데이트 수행.

### 3. 전체 데이터 재가공 (Full Re-trigger) 가이드
